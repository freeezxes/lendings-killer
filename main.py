...
def _ai_generate(data: dict) -> dict:
    ...
    if photo_urls:
        ...
        photos_block = f"\nПРИКРЕПЛЕННЫЕ ИЗОБРАЖЕНИЯ (фото работ, логотипы и т.д.):\nПользователь прикрепил эти изображения. Вставь эти теги <img> в подходящее место (например, если это логотип — в хедер, если фото работ — в портфолио). Если пользователь указал куда их ставить — следуй его указаниям. Не меняй сами ссылки (src)!\n{tags}"
    else:

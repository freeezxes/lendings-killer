...
async def chat(request: Request):
    ...
    try:
        session = await main.services.OnboardingService.upsert(user.id, session_id)
        session_id = session["id"]
    except main.domain.DraftLimitError:
        return main._api_error("Draft limit reached", 409, "draft_limit_reached")
    except main.domain.DraftConflictError:
    ...
    if not ready:
        session = await main.services.OnboardingService.upsert(
...
async def api_onboarding_generate(request: Request, background_tasks: BackgroundTasks):
    ...
    session = await main.services.OnboardingService.get(session_id, user.id)
    if not session:
    ...
    async with AsyncSessionLocal() as session:
        ...
        await session.execute(
            delete(models.onboarding.OnboardingSession).where(
                models.onboarding.OnboardingSession.id == session_id,
                models.onboarding.OnboardingSession.user_id == user.id
            )
        )
...
async def api_onboarding_status(request: Request, session_id: int):
    ...
    if status == "completed" and session.get("generated_site_id"):
        site = await main.services.OnboardingService._get_site(session["generated_site_id"])

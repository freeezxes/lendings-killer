...
async def auth_reset_page(request: Request):
    ...
    try:
        main.auth_services.PasswordResetService.validate(token)
...
async def auth_google_callback(request: Request):
    ...
    try:
        user_by_google = main.db.get_user_by_google_id(profile["google_id"])
        user_by_email = main.db.get_user_by_email(profile["email"])

        if user_by_google and user_by_email and user_by_google["id"] != user_by_email["id"]:
            main.logger.warning("Google OAuth account conflict: google_id and email map to different users")
            return main._auth_error_redirect("account_conflict")

        is_new_user = False
        if user_by_google:
            user = main.db.link_google_to_existing_user(user_by_google["id"], profile["email"], profile["google_id"], profile["avatar_url"], profile["email_verified"])
        elif user_by_email:
            user = main.db.link_google_to_existing_user(user_by_email["id"], profile["email"], profile["google_id"], profile["avatar_url"], profile["email_verified"])
        else:
            user = main.db.create_google_user(profile["email"], profile["google_id"], profile["name"], profile["avatar_url"], profile["email_verified"])
            is_new_user = True

        if not user:
            main.logger.warning("Google OAuth account linking or creation failed")
            return main._auth_error_redirect("account_conflict")
...
async def auth_register(
    ...
    try:
        ...
        user = main.auth_services.AuthService.register(
...
async def auth_login(
    ...
    try:
        main._verify_auth_csrf(request, csrf_token)
        user = main.auth_services.AuthService.login(
...
async def auth_forgot_password(
    ...
    try:
        main._verify_auth_csrf(request, csrf_token)
        reset = main.auth_services.PasswordResetService.request(
...
async def auth_reset_password(
    ...
    try:
        main._verify_auth_csrf(request, csrf_token)
        user = main.auth_services.PasswordResetService.reset(
...
async def auth_verify_email(request: Request):
    ...
    result = main.db.verify_email_token(request.query_params.get("token", ""))
    if not result.get("ok"):
        code = result.get("error", "verification_failed")
        return RedirectResponse(f"/auth?error={code}", status_code=302)

    user = main.db.mark_email_verified(result["user"]["id"])
    if not user:
        main.logger.warning("Email verification failed while marking user verified")
